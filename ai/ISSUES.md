...
- **The app's own JS path (react-native-performance marks, measurement build)**: `launch_native` 91–112 ms; `launch_js_bundle` 105–145 ms; ~308 ms from bundle end to `perf_monitor_init`; `js_to_content` 747–749 ms; `home_content` at ~6.15 s. The Hermes bundle is 4.4 MB of bytecode and mmap'd, so bundle load is not a cost, and no images ship in `assets/` at all (0 image entries in the APK) — neither parse nor asset decode owns any of the launch.
