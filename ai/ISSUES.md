...
### 25. [OPEN — scheduled for the follow-up session, owner directive 2026-09-09] Sound-sheet preview: first tap after a clip finishes naturally does nothing (audio + countdown); works on the second tap
...
- **Mechanism (code read, `components/sheets/screens/Sound.tsx`)**: the single-player refactor (G.4/G.5) keys `useAudioPlayer` on `playingSource = playingIndex !== null ? ATHAN_AUDIOS[playingIndex] : null`. On natural completion the finished-detector effect calls `setPlayingSoundIndex(null)`, so the source becomes `null` and the hook RELEASES the player instance. The next tap sets `pendingPlayRef = true` and `playingSoundIndex = index`; the arming effect (`[playingIndex, player]`) consumes the one-shot ref and calls `player.seekTo(0); player.play()` — but the source transition `null → clip` has just created a brand-new player whose source is still loading, and `play()` on the not-yet-loaded instance is a no-op. The ref is already consumed, so no code path retries. The second tap interacts with a now-loaded player and works. Repro B's flash is the selection indicator updating while playback itself no-ops.
- **Fix direction (for the follow-up session)**: either keep the player instance alive across natural completion (seek to 0 / pause instead of nulling the source, so `play()` always targets a loaded player), or arm playback from the status (play when `isLoaded` becomes true while `pendingPlayRef` is set) instead of firing once synchronously. Preserve the single-player invariant (G.4: never reintroduce per-row players) and the countdown semantics (`remainingSeconds` from status).
- **Files**: `components/sheets/screens/Sound.tsx` (player lifecycle, `handlePlayPress`, arming effect, finished-detector), `components/sheets/parts/SoundItem.tsx` (icon/countdown display only).
- **Verification**: both repros above on iOS and Android; manual-stop path unaffected; rapid row-switching during playback unaffected; sheet dismiss during playback stops audio (existing `onAnimate={clearAudio}`).

### 26. [OPEN — follow-up session] Android overlay dims the location/date header that iOS keeps bright; "London, UK" absent entirely on the Android overlay
