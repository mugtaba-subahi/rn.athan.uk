...
### 23. [OPEN — scheduled with the bug-fix session] Extras at-time notifications play the user's Athan selection instead of a distinct Extras sound
...
- **Owner spec (2026-09-08, final)**: ONLY the 5 daily prayers use the user's selected athan (Fajr, Dhuhr, Asr, Magrib, Isha). EVERYTHING else uses a single built-in separate audio: Sunrise (standard page, NOT one of the 5), Midnight, Last Third, Suhoor, Duha, Istijaba. The owner already created this audio file but has NOT imported it into the repository — locate it from the owner (likely in his Audacity source or local files; the GitHub Releases `audio-sources-v1` tag may hold it) and place it in `assets/audio/` before implementing.
- **Implementation sketch**: `getNotificationSound` and `genNotificationContent` gain schedule/prayer awareness — the 5 daily prayers map to `athan${soundIndex+1}.mp3` + `athanAndroidChannelId(soundIndex)`; all others map to the new fixed asset + a dedicated Android channel (remember channel sounds are immutable after creation — fresh channel id like the `athan_*_v2` pattern). Update `initializeNotifications` channel creation and the app.json `sounds` array for the new asset.

### 22. [FIXED 1.22.19 — device-verify pending] "Sunrise" wraps to two lines on the standard page (3T, EAS build)
