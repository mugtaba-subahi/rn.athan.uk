...
- **Timezone model is settled** - Prayer datetimes are true UTC instants (`createPrayerDatetime` = `fromZonedTime(..., PRAYER_TIMEZONE)`, `PRAYER_TIMEZONE = 'Europe/London'` in shared/constants.ts, the one place to change for v2.0); per-tick diffs are `target.getTime() − Date.now()`. Never reintroduce per-tick `createLondonDate()` — it lives only in sequence/display logic.
