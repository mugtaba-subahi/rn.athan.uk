...
- [2026-09-12] Cold-launch anatomy measured on the 3T (1.24.17, perf/launch-time, ISSUES #32): 6.6 s to first frame splits into **3.1 s of GMS `ProviderInstaller`** (the `modules/tls13` ContentProvider, Android ≤9 only), ~0.2 s of RN native init plus Hermes bundle eval, and **~1.9 s of JS module evaluation and React mount**. Attribution came from Android's event log (`am_proc_bound` → `am_on_create_called` → `Displayed`) plus atrace (`bindApplication` 3,752 ms, inside it `OpenDexFilesFromOat` 3,127 ms on `com.google.android.gms`) — `am start -W` alone only gives the total. **The TLS provider install must never move off the pre-`Application.onCreate` path**: ISSUES #21 proved a JS-side install fixes debug and fails release because okhttp snapshots `SSLContext.getDefault()` at client construction, and it is already a no-op on Android 10+, so the 3T is a worst case rather than the typical user. Forcing `cmd package compile -m speed` saved ~720 ms, all of it after onResume, dating that part to Java JIT warm-up rather than app work. The Hermes bundle (4.4 MB bytecode) is mmap'd and costs ~120 ms; no images ship in `assets/` at all. DURABLE LESSONS: (1) this shell's `ls` emits ANSI colour codes — capturing a path with `$(ls -t …)` yields an unusable filename and silently shifts `awk` column indexes, so use literal paths or `command ls`; (2) `strings` cannot see Hermes string literals (they live in a packed table), so grepping a bundle to prove a build flag compiled in returns a false negative — verify by runtime behaviour instead.

- [2026-09-11] Phones in other timezones, the Jan 1 re-download and the long-suspension stop-gap (1.24.14, fix/year-boundary, ISSUES #30, #4, #31): calendar days now follow `PRAYER_TIMEZONE` everywhere — read by Intl from the instant and carried as YYYY-MM-DD strings — and `createPrayerDatetime` no longer depends on the phone's timezone. The full suite had failed 36/9/32 tests with the phone in New York/Tokyo/UTC+14; now 997/997 in all four zones of `yarn test:tz`. A data refresh keeps yesterday's record, so Jan 1 no longer downloads last year again for Dec 31. After a suspension longer than the 3-day buffer, the stop-gap list rebuilds from today. Deliberate breaks each caught (8 and 34, 2, 1, 1 failures). DURABLE LESSON: date-fns-tz's `toZonedTime`, `getTimezoneOffset` and `fromZonedTime` rebuild Dates on the phone's clock and can be an hour off when that clock skips or repeats an hour; read another timezone's clock with Intl `formatToParts` on the instant.

- [2026-09-11] Extras night times and alert moments (1.24.12, fix/night-times, ISSUES #29): the owner confirmed, after research, that the Islamic date begins at Maghrib and a night belongs to the day that follows it, so each night uses the Maghrib that starts it and the Fajr that ends it (ADR-004 already said so; the code had drifted). **Fixed:** (1) the Extras list for D showed the night *after* D's values on the night *before* D — 0–2 min off on ordinary days, 30/39 min on the night before a clock change; (2) clock-change nights now use the real elapsed night (owner request); (3) notifications and reminders fire at the list row's own instant through `getPrayerForDate` — before, a winter Midnight alert fired a night after its row and a phone in another timezone fired at its own clock time. Midnight/Last Third are no longer stored (the #5 Dec 31 patch and the #28 string helpers are gone); `PRAYER_TIMEZONE` names the timezone once for v2.0. Tests: a full real year (2024) against an independent reference and the 2026 clock-change nights from the live API; deliberate breaks each caught (13, 10 and 2 failures). DURABLE LESSON: when one moment is shown in the UI and fired as an alert, derive both from one function — two derivations of the same moment drift apart silently.

- [2026-09-11] REVALIDATION on Opus 5 (1.24.7, fix/revalidation-2026-09-11): the owner found the 2026-09-10 sessions had run on Sonnet 5 and ordered a full re-audit and re-plan. Method: three independent Opus reviewers (one per commit group), an unanchored Opus architect, direct source and GitHub checks, and on-device runs on the OnePlus 3T. **Wrong in the record:** (a) The root cause. Upstream #9574 is a third-party report (Reanimated 4.4.0 / RN 0.85.3) of the default-on settled-props sync committing a stale snapshot after resume; PR #9527 fixed it in the C++ `AnimatedPropsRegistry` and the JS props GC, and the fix is present in the installed 4.6.0 and absent from 4.5.1. `NodesManager.kt` is byte-identical in both, and its `mCallbackPosted.set(true)` in `onHostPause` is the marker that restarts in-flight animations on resume, not a defect. (b) The `[resync]` dependency arrays (1.24.1 on the five consumer hooks, 1.24.3 on Countdown): Reanimated 4.6 ignores that argument on native (web only), so they did nothing except log a DEV warning on every render. (c) 1.24.3's alert-bounce resume guard: assigning `scaleX`/`scaleY` mid-dip cancels the sequence, the dip callback gets `finished=false` and the glyph swap never runs, so the bell could keep the old icon; without the guard the bounce simply finishes on resume. (d) 1.24.5 restored the cascade to 1000ms but missed the next-prayer advance, which ADR-015 had also cut from 1000ms to 150ms. (e) 1.24.3's Ago rewrite lost the snap on the first ready data. (f) 1.24.6's sync tests still passed with the bug re-introduced. (g) 1.24.4's premise: on real London data the canonical and chronological Extras orders are always equal (full-year sweep, 65,797 lists); the mismatch exists only on the mock rig. **1.24.7:** dead `[resync]` arrays removed (`useAnimation.ts` and `Countdown.tsx` back to 1.24.0); bounce guard reverted to 1.24.0; `useDerivedProgress` latches its timing options when the target changes, so a re-render mid-transition cannot restart it with other timing; row colour timings back to pre-ADR-015 (selection 150ms, next-prayer advance and cascade 1000ms, stagger unchanged); countdown-bar overlay fade linear again; Ago mounts its coloured badge only once ready; overlay and pill positions come from `canonicalDisplayOrder` (the order List renders) and the explanation text is keyed by name; sync tests use a distinct Fajr per record and full-date lookups, and fail on own-Fajr, wrong-year and removed-guard mutations; the countdown test no longer leaks a mock; `e2e/flows/sounds-x5.yaml` uses one BACK ("Change athan" closes Settings in the same tick). **Verified on the 3T (1.24.6):** the Extras explanations match all four tapped rows; resume across a boundary that passed while suspended leaves the overlay closed and every row, the pill and the bar correct. **1.24.8:** idle CPU on the 3T bisected to 19.1% at 1.23.13 and 75–85% from 1.23.15 on, almost all main thread — 1.23.14 had brought back a 1000ms linear `withTiming` re-issued every second on the bar's width; the bar now sets sub-pixel per-second steps directly (still written every tick) and animates only visible steps, the refill and the warning flip. **1.24.9:** the night helpers compute from the HH:mm strings alone (ISSUES #28 fixed; bit-identical to the old code on ordinary days across 201,600 Maghrib/Fajr pairs). **Owner decisions (2026-09-11):** merged to uat after the owner's test (1.24.10); keep Reanimated 4.6.0 + worklets 0.12.2; What's New re-stamped to 1.24.10 (the three 1.23.15 items were never released); #29 — the Extras night leading into day D is computed from D−1's Maghrib and D's Fajr (its own branch); iOS check on the XS pending. Full report and re-plan: `ai/features/revalidation-2026-09-11/REPORT.md`. DURABLE LESSONS: verify a cited upstream fix against the actual diff, not the issue title; confirm which build is installed before measuring; pipe-to-`tail` hides a failed build's exit code; a helper that builds times from "now" makes cached data depend on when it was fetched.

- [2026-09-10] [PART (2) SUPERSEDED 2026-09-11 by ISSUES #29: night times are no longer stored, so the Dec 31 correction was removed] Date-roll cascade fade restored to full duration + Dec 31 derived-time correction (1.24.5/1.24.6, fix/prayer-list-followups, device + unit verified): two owner-reported follow-ups from the presentation-rearchitecture verification pass. **(1) Cascade fade snap**: the owner noticed the day-roll cascade (staggered per-row color fade) "used to be smooth, now snaps." Root-caused via git history — the pre-ADR-015 effect (`AnimColor.animate(0, { delay })`, no explicit duration) fell back to `ANIMATION.durationSlow` (1000ms); ADR-015's `useDerivedColor` replacement unified the cascade and ordinary-selection cases onto one explicit `duration: ANIMATION.durationFade` (150ms), silently cutting the cascade to a near-snap. Fixed identically in `Prayer.tsx`/`Time.tsx`/`Alert.tsx`: `duration: isCascadeRoll ? ANIMATION.durationSlow : ANIMATION.durationFade`. Device-verified with a script-measured 20fps frame analysis of a live day-roll: 5/6 rows show a genuine 650-1200ms transition staggered at ~150ms apart (not a 1-3 frame snap); the fade direction turned out to be bright→dim (outgoing rows dimming as the new day's first prayer stays bright), not dim→bright as initially assumed — cosmetic correction to the mental model only, the timing verdict is unaffected. The active-pill slide was cross-checked in the same capture and is a clean, un-glitched ease slide (no bug there). **(2) Dec 31 derived-time correction (ISSUES #5)**: `transformApiData` computes Midnight/Last-Third from next-day Fajr but has no cross-year awareness, so a year's last entry falls back to same-day Fajr — confirmed this never self-heals. Added `correctYearBoundaryDerivedTimes` (pure, `shared/prayer.ts`) + `fixYearBoundaryDerivedTimes` orchestration (`stores/sync.ts`), wired into all three year-boundary fetch scenarios (December next-year-only, December both-years, Jan-1 previous-year). 9 new unit tests, not device-verified (impractical to trigger live; this is core prayer-time data so correctness rests on the unit tests per owner directive — "heavily tested" was the explicit bar, not a device pass). DURABLE LESSON: when an animation-mechanism rewrite unifies several call sites onto one shared parameter set (here, cascade and selection color both routed through one `duration` argument), audit every call site's *previous* implicit defaults individually — a clean unification can silently drop a value that was never meant to be shared.

- [2026-09-10] Extras overlay showed the wrong prayer's explanation popup and could mis-position the active pill (1.24.4, device-verified S23, found live by the owner during the presentation-rearchitecture verification pass): `overlay.selectedPrayerIndex` is the selected prayer's position in the **chronological** sequence array (matches `usePrayer`/`getOverlayTarget` elsewhere), not its on-screen row — Extras rows display in **canonical** rank order (Midnight, Last Third, Suhoor, Duha, Istijaba; see `canonicalDisplayOrder` in `shared/prayer.ts`), which only equals the chronological index by coincidence. Two call sites assumed they were the same number: `components/overlay/Overlay.tsx` indexed `EXTRAS_ENGLISH`/`EXTRAS_EXPLANATIONS`/`EXTRAS_EXPLANATIONS_ARABIC` directly by `selectedPrayerIndex` (wrong prayer's name/explanation shown, and the info-box position and the press-catcher's row-exemption geometry were computed from the same wrong number — not just wrong text, wrong screen position too), and `components/prayer/ActiveBackground.tsx` computed the active pill's `yPosition` the same way (a latent bug that hadn't visibly triggered yet, since the specific data at hand happened to have the actual next prayer's chronological and canonical positions coincide). Owner's repro: tapping Midnight/Last Third/Suhoor each opened the popup for the *next* canonical entry, wrapping Suhoor back to Midnight; Duha (whose chronological and canonical index happened to match) looked correct, masking the bug until tested by hand. Fix: both sites now resolve the prayer's actual visual row from its English name (`EXTRAS_ENGLISH.indexOf(...)`) before using it for any position or content lookup; Standard is untouched (no canonical reordering exists there, so its index was already correct). Verified device-side: all 4 extras rows (Midnight, Last Third, Suhoor, Duha) tapped individually, hero/row/popover text confirmed matching in every case. DURABLE LESSON: `selectedPrayerIndex` is chronological-sequence-shaped, never assume it equals a canonical/display row position for Extras — resolve through the prayer's name when a visual position is needed.

- **[CORRECTED 2026-09-11: the root-cause mechanism and the `[resync]` claims in this entry are wrong — see the 2026-09-11 entry]** [2026-09-10] Presentation-layer re-architecture root cause CONFIRMED and FIXED (1.24.2/1.24.3, experiment/presentation-rearchitecture, device-verified S23): a dedicated research-and-design session (`ai/features/presentation-rearchitecture/BRIEF.md`) traced the whole stranded-veil/empty-pill/dim-icon/stale-bar bug family to a real, upstream-acknowledged native defect, not an inherent Reanimated limitation. **Root cause**: `node_modules/react-native-reanimated/android/.../NodesManager.kt`'s `onHostPause()` clears its "animation running" flag to stop the frame callback, then immediately sets it back to `true` — so `isAnimationRunning()` lies for the whole backgrounded window, gating off the synchronous prop-flush fallback in `NativeProxy.kt`'s `maybeFlushUIUpdatesQueue()` and deferring any prop write to the next real frame on resume, where it can race a 2-second `settledProps` GC eviction. Filed upstream as [reanimated#9574](https://github.com/software-mansion/react-native-reanimated/issues/9574) against this app's exact stack, fixed by PR #9527, released in 4.5.3 and inherited by 4.6.0. Separately confirmed: the app's own `resync`-counter mapper-restart trick (`hooks/useAnimation.ts`) cannot force a re-apply of an unchanged value by itself — `forceUpdate` is real but only Reanimated's own mount/attach logic ever sets it `true`; a restarted mapper recomputes against the same stored `oldValues` and is skipped by the identical `shallowEqual` gate if the target didn't change. **Fix, layered**: (1) `react-native-reanimated` 4.5.1 → **4.6.0** + `react-native-worklets` 0.10.1 → **0.12.2** (its required peer pair; 4.5.3 alone would have been the narrower change but the owner chose the latest paired release) — patches the confirmed native defect directly. (2) Closed 3 remaining settled-value gaps that predate this session and were never covered by ADR-015's derived-hook migration: `components/countdown/Countdown.tsx`'s hero scale/translateY (a **previously undiscovered** instance of the same bug — local `useAnimatedStyle` with no `resync` dependency at all), `components/prayer/Ago.tsx`'s recent-color state (was effect-written via `useEffect` + `useSharedValue`, now `useDerivedColor`/`useDerivedBackgroundColor` matching the established pattern, net code deletion), and `app/Navigation.tsx`'s pager-dot opacity (was imperative `.animate()` in the page-select handler, now derived from a `currentPage` state value via `useDerivedOpacity`). (3) `hooks/useAlertSwapBounce.ts` gained a narrow resume-guard (a ref flag set for the bounce's duration, cleared by the spring's own completion callback; a `resync` firing while still set snaps `scaleX`/`scaleY` to 1) for the one remaining genuinely event-driven risk — backgrounding mid-bounce. DURABLE LESSON: `SharedValue.modify()` is the one public, documented API that bypasses the "unchanged value" skip, but it only guarantees the *mapper* reruns (via its listener chain), not that `updateProps` fires — the mapper rerun is still a zero-argument call into `styleUpdater`, so `.modify()` is not a substitute for a real value change or a correct dependency array. **Verification**: `yarn validate` green (tsc, biome, 966/966 jest); device-verified on the Samsung Galaxy S23 (Android 16) with paired before/after screenshots read by a vision-analysis subagent doing actual pixel-diffs (not visual impression) — two independent overlay-open scenarios (a non-next row with no pill, and the actual next prayer carrying the active pill) came back byte-for-byte identical (0 differing pixels, MD5-identical in one pair) across 10-30 second backgrounded windows; the pager-dot fix confirmed via a real page swipe. No residual staleness was found after the 4.6.0 bump, so the contingent app-level `.modify()` hardening (evaluated as a last-resort layer) was not needed. **Background/foreground ticking, frozen decision**: continuous background ticking is not achievable here without unacceptable store-policy risk — researched and sourced against official RN/Android/Reanimated source (JS timers and Reanimated's own UI-thread mapper loop stop within about one activity transition of backgrounding on both platforms, well before OS-level suspension/freezing even applies), Apple App Review 2.5.4 and Google Play's foreground-service policy (a foreground service or an iOS background-audio trick with no genuine matching functionality is a recognized rejection pattern), and current community-library maintenance status (`notifee` archived, `react-native-background-actions` has an open unresolved New Architecture issue and an inactive maintainer). Instant, correct catch-up on foreground is the real ceiling — this is now what the fix above delivers, not a fallback. **New findings, out of scope for this fix, tracked separately**: (a) a day-roll transition can leave the prayer list showing only one row (the new day's Isha) instead of all six for a period after the sequence cascades past today's final prayer, self-resolving as the sequence advances further into the new day — traced to `components/prayer/List.tsx:33`'s `belongsToDate === displayDate` filter, not a Reanimated/animation issue at all; root cause not yet found (see `ai/ISSUES.md`). (b) the date-roll cascade fade (`getCascadeDelay` in `shared/prayer.ts`, consumed via `useDerivedColor`'s `delay` option in `Prayer.tsx`) was independently confirmed by the owner as feeling harsher/less smooth than before, but owner-confirmed **pre-existing and unrelated to this session's changes** — no code in this fix touches `getCascadeDelay` or the cascade's easing.

- **[CORRECTED 2026-09-11: the consumer-mapper `[resync]` fix in this entry does nothing on native — see the 2026-09-11 entry]** [2026-09-10] Overlay resume hardening: alert-icon dim + countdown-bar catch-up FIXED (1.24.1, feat/overlay-rewrite): two resume bugs, one root. (1) **Alert icon dimmed on resume** while the English/Arabic name and time stayed bright: Reanimated re-applies an animated style or prop only when its mapper runs (a shared-value change or a mapper restart), never on a plain re-render, and a derived snap to an unchanged value is a no-op. The SVG alert `fill` exists only through `animatedProps`, so it was not re-asserted on foreground and could stay dim. Fix: every consumer mapper in `hooks/useAnimation.ts` now passes the `resync` counter to `useAnimatedStyle`/`useAnimatedProps`, so the mapper restarts and re-applies on resume. (2) **Countdown bar animated a slow catch-up** from its pre-suspend width: the width effect animated the large resume jump, and JS timers are frozen in the background. Fix: `resyncCountdowns()` (`stores/countdown.ts`) recomputes the countdown atoms and catches up a boundary crossed while suspended, and `Bar.tsx` snaps (no `withTiming`) on a resume bump. `device/listeners.ts` runs `checkOverlayBoundary` then `resyncCountdowns` then `bumpResync` on every foreground. DURABLE LESSONS: a prop that exists only via `animatedProps` (SVG `fill`) must take the resume counter in its mapper dependencies; never rely on background JS timers (iOS suspends the process, Android freezes cached processes). A DEDICATED SESSION is chartered to research continuous background/foreground ticking ("the UI must never stop") — see the session handoff.

- [2026-09-10] Overlay RE-ARCHITECTURE rewrite (1.24.0, feat/overlay-rewrite, owner-approved on the XS; ADR-015, spec `ai/features/overlay/spec.md`): ADR-014's in-place overlay was gutted and rebuilt on derived state. Every overlay-visible attribute is now a pure function of atoms rendered through the new derived hooks (`useDerivedProgress`/`Opacity`/`Color`/`BackgroundColor`/`TranslateY`/`Fill` in `hooks/useAnimation.ts`), which re-run on a target OR resume-counter change and snap on mount and resume. No `useEffect` writes an animation target on the overlay path, so a suspend-dropped write can no longer strand. The open state changes only on the owner's tap, app close, and a wall-clock 2s schedule deadline (`armOverlayBoundary`/`checkOverlayBoundary` in `stores/countdown.ts`, enforced before `sync()` in `device/listeners.ts`); opening is refused against the true remaining milliseconds. `refreshUIAtom` became the resume counter `resyncAtom`. Non-selected rows are hidden from the screen reader while open. Pixel parity is a hard rule (same durations/easings; iOS shadow and Android API 29 `boxShadow` kept separate). DURABLE LESSONS: (a) never pass `easing: undefined` explicitly to `withTiming` — it overrides Reanimated's default and aborts with "undefined is not a function" on the first non-snap evaluation, which the mount snap hides; (b) an animated value that derives from state cannot strand, an animated value written by an effect can; (c) `resync` is for host resumes only, never for state transitions such as a date roll (those derive from state). Follow-up (separate branch): generalize the derived-plus-resume pattern to the remaining effect-driven animators (Bar width/warning, Ago recent colour, Navigation dots) and consider renaming `resyncAtom` to an intent name.

- [2026-09-10] Pre-boundary overlay lock REINTRODUCED + overlay-selected row stays bright on resume (1.23.15, fix/resume-strand-fixes, owner-tested S23): ADR-014 replaced the ≤2s lock with selection-follows-next-prayer. That only works while the overlay can straddle a boundary, and the straddle is the source of the stranded states the owner kept hitting: a boundary crossed with the overlay open (especially while the host was suspended) leaves a stale selection, and the advance/veil writes issued in the wind-down window drop, stranding holes and the pill. Owner directed the simpler proven model back: `canShowOverlay` refuses open and selection while the page countdown reads ≤2s, the sequence ticker closes the overlay as it enters the final 3s, and the boundary branch closes it too so a suspended crossing never straddles. The advance helpers and their specs are deleted; auto-close specs added. Separately, the `refreshUI` color effects in `Prayer`/`Time`/`Alert` now write `isSelectedForOverlay ? 1 : initialColorPos` (they overwrote a selected-bright row with its natural dim color), and the row/pill veils re-issue on `refreshUI` (the tick's boundary close is an outside-React veil flip, the same dropped-write class as the pill slide). `createInstant` keeps millisecond precision: second truncation makes `isPassed` false for a just-passed prayer in the boundary second, so a mount or the resume color write settles that row dim. DURABLE LESSON: removing that precision during this session's "cleanup" reintroduced the dim bug within minutes of owner testing. Do not treat the millisecond precision as removable while any effect writes `initialColorPos`.

- [2026-09-10] Resume-animation strand + overlay-passed-row dim FIXED (1.23.14, fix/resume-strand-fixes, S23-verified with frame evidence): four coordinated fixes. (1) The active pill (`ActiveBackground.tsx`) re-issues its slide on `refreshUIAtom` like the rows do — a boundary crossed while the host was suspended stranded the pill on the old row forever because `yPosition` never changes again after resume. (2) The countdown bar returns to raw per-second progress with a per-second linear `withTiming` (`stores/countdown.ts`, `useCountdownBar.ts`, `Bar.tsx`); the perf campaign's quantized direct-set removed the bar's one-tick self-heal, so a resume-dropped width write stayed stale for minutes. (3) The Android depth shadow moved onto the pill's own view so it travels with the slide; the row-anchored shadow keyed to `isNext` snapped to the new row a full second before the pill arrived (API >= 29 gate unchanged, iOS shadow untouched). (4) **`createInstant` now keeps millisecond precision** — the wall-clock ticker commits the boundary transition in the first ~100ms after :00, where a second-truncated "now" EQUALS the prayer datetime, so `datetime < now` read the just-passed prayer as still future and the deselection effect painted its row dim (stranded until the next external re-render; resume healed it). DURABLE LESSONS: (a) any clock read that feeds a `<`/`>` prayer comparison must carry sub-second precision when the consumer commits at :00.0x — the old countdown-down tick masked the truncation because its boundary commit ran a full second late; (b) a one-shot effect keyed on a boolean (selection) never re-fires when its data input corrects later, so effects that write animation targets must either read fully-corrected data or re-fire on the data too — this is the same class as the still-open overlay-dim-on-resume bug (see next session's handoff: the `refreshUI` effects overwrite a selected-bright row with its natural dim color because they ignore `isSelectedForOverlay`).

- [2026-09-09] Audio 30s iOS-cap audit CLEARED, no changes shipped: all 99 audio files measure under the iOS 30-second custom-sound cap by decoded duration — longest athan15 at 29.975s, longest reminder 22.85s — and athan15 was verified AUDIBLE on the XS (the longest file passing clears every shorter file; the check is monotonic). DURABLE LESSON: MP3 "duration" depends on the measure — decoded PCM (Audacity/ffmpeg, honors LAME gapless trimming), CoreAudio estimate (afinfo, ~+1 frame), raw MPEG frame count (mp3-duration, +2-3 frames of encoder padding ≈ +50-70ms). mp3-duration OVERCOUNTS: athan15 reads 30.041s by frame count but decodes to 29.975s. Any future audit flagging a notification sound ">30s" must re-verify with `ffmpeg` decoded duration before believing it (this session's 30.041s flag was a false alarm). `ATHAN_DURATION_SECONDS` stays pinned to mp3-duration values — internally consistent with its test, but the table runs ~50-70ms hot by design.

- [2026-09-09] ISSUES #25 sound-preview first-tap FIXED (1.23.2, fix/sound-preview-first-tap, owner-verified XS + 3T): **expo's `useEvent` (under `useAudioPlayerStatus`/`useVideoPlayerStatus`) keeps the LAST event payload in useState across shared-object instance swaps** — when `useAudioPlayer`'s source changes, the hook releases and recreates the player but the status hook serves the DEAD instance's terminal payload until the new one emits (a null-source player emits nothing, so after natural completion the stale `playing:false, currentTime≈duration` status persisted forever). The sheet's finished-detector then reaped every freshly armed player in the same commit — the whole bug. Guard: compare `status.id !== player.id` (every payload carries the player UUID on both platforms) before trusting status. Countdown hardening in the same fix: `Math.round` not floor (clip durations are non-round 19.3-30.0s and iOS reports a provisional duration before refining — floor flashed one second low at start), plus `ATHAN_DURATION_SECONDS` (rounded clip lengths parallel to `ATHAN_AUDIOS`, regenerate when audio changes) standing in until the fresh player reports so the countdown appears in the same frame as the icon flip (data latency was ~50-150ms + a 75ms fade that read as desync; fade removed owner-approved, color tween kept). DURABLE LESSON: any effect consuming an expo shared-object status must bound it to the current instance identity — stale payload survives emitter replacement by design in `useEvent` (confirmed by expo/expo#43136; recreate-on-source-change is intentional per maintainers, so the id-guard is the canonical defense until upstream changes `useEvent`). The duration table is pinned by `shared/__tests__/athanDurations.test.ts` (mp3-duration devDep; regenerate the table when audio changes or the test reds with the correct values).
