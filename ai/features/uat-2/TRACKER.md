...
| 23 | Merge uat-2 → uat (on the owner's word) | `uat` | ⏸️ **deferred 2026-09-12 by the owner** — "I don't want to merge UAT 2 into UAT for a little while longer... I've thought of some more things we want to address." The upgrades programme (`ai/features/upgrades-2026-09/BRIEF.md`) and the code audit come first |
| 24 | Update the alarm clock branch from uat | `experiment/alarmclock-backport` | ⏸️ deferred with 23. Expect real conflicts when it happens: the backport carries its own notification and background-schedule patches in `stores/notifications.ts`, the same file #34 changed |
