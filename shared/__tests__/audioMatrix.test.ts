 ...
 * but it is SHIPPED by four hand-maintained surfaces: the assets directory,
 * `app.json`'s expo-notifications `sounds[]` array, Android's `res/raw`, and the
 * iOS bundle. Nothing checked that they agree.
 *
...
import { EXTRAS_NOTIFICATION_SOUND } from '@/shared/notifications';
...
/** Mirrors prayerNameSlug in shared/notifications.ts, which is private to that module */
const slug = (englishName: string): string => englishName.toLowerCase().replace(/\s+/g, '_');
...
describe('the audio matrix closes across every surface', () => {
  ...
  it('ships exactly those files in android res/raw', () => {
    expect(mp3sIn(ANDROID_RAW_DIR).sort()).toEqual(EXPECTED);
  });

  it('ships exactly those files in the iOS bundle', () => {
